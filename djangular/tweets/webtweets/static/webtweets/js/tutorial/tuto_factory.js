tutoApp.factory('tutoFactory', function() {
    var tutoData = [{
        "id": 1,
        "type": "Condo",
        "price": 220000,
        "address": "213 Grove Street",
        "description": "Excellent place, really nice view!",
        "details": {
            "bedrooms": 2,
            "bathrooms": 1.5,
            "area": 921
        },
        "image":"tuto-1"

    }, {
        "id": 2,
        "type": "House",
        "price": 410500,
        "address": "7823 Winding Way",
        "description": "Beautiful home with lots os space for large family.",
        "details": {
            "bedrooms": 4,
            "bathrooms": 3,
            "area": 2145
        },
       "image":"tuto-2"

    }, {
        "id": 3,
        "type": "Duplex",
        "price": 395000,
        "address": "834 River Lane",
        "description": "Great neighbourhood and lot's of nice green space.",
        "details": {
            "bedrooms": 3,
            "bathrooms": 2.5,
            "area": 1500
        },
        "image":"tuto-3"

    }, {
        "id": 4,
        "type": "House",
        "price": 755990,
        "address": "7809 Forest Avenue",
        "description": "Best house on the block!",
        "details": {
            "bedrooms": 6,
            "bathrooms": 4.5,
            "area": 3230
        },
        "image":"tuto-4"

    }, {
        "id": 5,
        "type": "Condo",
        "price": 210500,
        "address": "1857 Andover Court",
        "description": "Nice little condo with room to grow.",
        "details": {
            "bedrooms": 2,
            "bathrooms": 1.5,
            "area": 1023
        },
        "image":"tuto-5"

    }, {
        "id": 6,
        "type": "House",
        "price": 334900,
        "address": "7398 East Avenue",
        "description": "You'll love the view!",
        "details": {
            "bedrooms":4,
            "bathrooms": 2.5,
            "area": 1788
        },
        "image":"tuto-6"
    }];

    function getTuto() {
        return tutoData;
    }
    return {
        getTuto: getTuto
    }
});


// tutoApp.factory('tutoFactory', function($http) {
//     var function  getTuto(){
//         return $http.get('data/data.json')
//     }
//     function getTuto(){
//         return tutoData;
//     }
//     return{
//         getTuto:getTuto
//     }
// });
